    ...
    // Document-layer constants moved to assets/js/modules/documents.js (Phase 2D-2).
...
${answer}` : action;
    ...
    async function handleDocSelect(event) {
        ...
        const projectId = window.CinoCodeState && window.CinoCodeState.project ? window.CinoCodeState.project.activeProjectId : null;
        if (!projectId) {
            if (typeof showNonBlockingToast === 'function') showNonBlockingToast('Proje seçili değil.');
            event.target.value = '';
            return;
        }

        for (const file of files) {
            if (window.CinoCodeDocuments && typeof window.CinoCodeDocuments.parseAndAddDocument === 'function') {
                await window.CinoCodeDocuments.parseAndAddDocument(projectId, file);
            } else {
                if (typeof showNonBlockingToast === 'function') showNonBlockingToast('Belge okuyucu modülü yüklenmemiş.');
            }
        }
        event.target.value = '';
    }
...
Object.defineProperty(window, 'projects', {
 ...
 // Projeler ekranında açık olan proje (null = proje grid'i)
    ...
    window.addEventListener('cinocode:project-deleted', () => {
        ...
        Promise.allSettled([
            ...
            (async () => {
                ...
                repairBadChatTitles();
                checkOllamaStatus();
                window.ollamaCheckInterval = setInterval(checkOllamaStatus, 5000);
    ...
    let ollamaCheckFailures = 0;
    const OLLAMA_MAX_FAILURES = 3;
    const OLLAMA_RETRY_DELAY_MS = 30000;

    async function checkOllamaStatus() {
        try {
            const res = await fetch(getOllamaUrl() + "/", { method: "HEAD", signal: AbortSignal.timeout(5000) });
            if (res.ok) {
                document.getElementById("statusIndicator").classList.add("online");
                ollamaCheckFailures = 0;
            } else {
        ...
        if (ollamaCheckFailures >= OLLAMA_MAX_FAILURES) {
            clearInterval(ollamaCheckInterval);
            setTimeout(() => {
                ollamaCheckFailures = 0;
                ollamaCheckInterval = setInterval(checkOllamaStatus, 5000);
