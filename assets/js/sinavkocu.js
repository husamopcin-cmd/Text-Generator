...
function buildExamCoachSuffix() {
    ...
    // Auth functions imported from auth-core.js
    const normalizeLocalProfileName = window.CinoCodeAuth?.normalizeLocalProfileName || ((value) => {
        return String(value || '')
            .replace(/[\u0000-\u001f\u007f<>]/g, '')
            .replace(/\s+/g, ' ')
            .trim()
    ...
    function forgetLocalProfile(name) {
        ...
        const profiles = (window.CinoCodeAuth?.getLocalProfiles || (() => []))();
        saveLocalProfiles(profiles.filter(item => item.toLocaleLowerCase('tr-TR') !== normalized));
    }

    function localProfileExists(name) {
        const normalized = normalizeLocalProfileName(name);
        if (!normalized) return false;
        if ((window.CinoCodeAuth?.localProfileExists || (() => false))(normalized)) return true;
    ...
    function renameLocalProfile(newNameRaw) {
        ...
        if (newName.toLocaleLowerCase('tr-TR') === (loggedUser || "").trim().toLocaleLowerCase('tr-TR')) {
            (window.CinoCodeAuth?.rememberLocalProfile || (() => {}))(newName);
            return true;
        }
    ...
    function openLocalProfileModal() {
        ...
        box.innerHTML = `
            ...
            <button id="localProfileDeleteBtn" style="width:100%; background:rgba(243,139,168,0.1); border:1px solid rgba(243,139,168,0.3); color:var(--cc-red); cursor:pointer; font-size:12px; font-weight:600; padding:8px; border-radius: var(--cc-radius); margin-bottom:8px;">🗑️ Yerel Verileri Sil</button>
